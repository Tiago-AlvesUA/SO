...
//CRIAR FC PARA USAR NAS THREADS void* void*
...
//FUNCAO PARA USAR EM THREADS -> como passar 2 args do client para criar fc com void* de arg
// usar struct ARGV
...
int main(int argc, char *argv[])
{
    ...
    int option;
    while ((option = getopt(argc, argv, "n:s:c:h")) != -1) //letras sao args q o programa aceita (usados com -*letra*(*arg extra*))
    {
        switch (option)
        {
            case 'n':
    ...
    {
		thread_create(&server_threads[i], NULL, &server_main, &server_args[i]);
	}
    /* 
     * TODO point
	...
	{
		thread_create(&client_threads[i], NULL, &client_main, &client_args[i]);
	}
    /* 
     * TODO point 
