...
namespace fifo
{
    ...
    void in(uint32_t id, uint32_t value)
    {
        ...
        fifo->ii = (fifo->ii + 1) % FIFOSZ; //aumenta point of insertion até
        // ao tamanho total do fifo e depois volta a por ii na posição 0.
        fifo->cnt++;
    }
    ...
    void out(uint32_t * idp, uint32_t * valuep)
    {
        ...
        /* Retrieve pair */
        *valuep = fifo->slot[fifo->ri].value; //retira valor e coloca 99999
        fifo->slot[fifo->ri].value = 99999;
        *idp = fifo->slot[fifo->ri].id; //retira id e coloca 99
        fifo->slot[fifo->ri].id = 99;
        fifo->ri = (fifo->ri + 1) % FIFOSZ; //incrementa retrieval point
        fifo->cnt--;
    }
